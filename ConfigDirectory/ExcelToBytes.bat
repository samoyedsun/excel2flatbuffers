...
if not "%1"=="" (
    ...
    %PROGRAM_DIR%\Tools.exe "%PROGRAM_DIR%\metadata.json" "%PROGRAM_DIR%\%~n1.bfbs" "%EXCEL_DIR%\%~nx1" "%TARGET_DIR%\%~n1.bytes" 3
) else (
	:: 批量导出模式（无参数时）
	for /f %%i in ('dir /b "%EXCEL_DIR%\*.xlsx"') do (
		echo 导出文件 %%~nxi 为 %%~ni.bytes 通过 %%~ni.bfbs
		%PROGRAM_DIR%\Tools.exe "%PROGRAM_DIR%\metadata.json" "%PROGRAM_DIR%\%%~ni.bfbs" "%EXCEL_DIR%\%%~nxi" "%TARGET_DIR%\%%~ni.bytes" 3
	)
)
